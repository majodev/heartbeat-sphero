...
var DataStream = exports.DataStream = function DataStream() {
  this.sensors = [];
  return this;
};
DataStream.prototype.load = function(sensors) {
  this.sensors = sensors;
};
DataStream.prototype.translate = function(msg) {
  console.log(msg);
  var ret = [];
  var data = msg.DATA;
  for (var i = 0; i < data.length; i += 2) {
    var slice = data.slice(i, i + 2);
    var val = slice.readInt16BE(0);
    ret.push(val);
  }
  return ret;
};
...
require('util').inherits(SpheroCollection, require('eventemitter2').EventEmitter2);
SpheroCollection.prototype.connect = function() {
  var self = this;

  var setupConnection = function(dev){

    console.log("setupConnection - dev:" + dev);

    var b = new Sphero(dev);
    b.on("open", function(err, ret){
      b.getBluetoothInfo(function(ret){
        console.log("bluetooth info", ret);
        self.balls.push(b);
    ...
    b.on("notification", function(msg) {
      self.emit('notification', msg);
    });

    // added to emit error events also...
...
SpheroCollection.prototype.ping = function(callback) {
  this.balls.forEach(function(ball) {
    ball.ping(callback);
  });
};
SpheroCollection.prototype.setDataStreaming = function(sensors) {
  this.balls.forEach(function(ball) {
    ball.setDataStreaming(sensors);
  });
};
SpheroCollection.prototype.setStabilization = function(shouldStabilize) {
  this.balls.forEach(function(ball) {
    ball.setStabilization(shouldStabilize);
  });
};
...
var Sphero = function(dev){
  ...
  this.dev.on('error', function (e) {
    ...
    // edit majo: emmitting the error does produce an unhandles exception in serial emitter!
    // Uncaught Error: Uncaught, unspecified 'error' event. from eventemitter2.js
    self.emit('error'); 
...
SpheroCollection.prototype.sensors = {
  gyro_h: {
    MASK1: 0x000000001
  },
  gyro_m: {
...
Sphero.prototype.setDataStreaming = function setDataStreaming(sensors) {
  this.setDataStreaming(sensors, 0, 0, 0, null);
};
Sphero.prototype.setDataStreaming = function setDataStreaming(sensors, hertz, frames, count, callback) {
  ...
  var buffer = new Buffer([0,0,  0,0,  0,0,0,0,  pcnt,  0,0,0,0]);
